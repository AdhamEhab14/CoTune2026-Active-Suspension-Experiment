...
const SESSION_DURATION = 390000; // 5 minutes in milliseconds
...
const useQueueStatus = () => {
  ...
  useEffect(() => {
    if (isAllowed && timeRemaining === 0 && identityId && (users.length > 0 && users[0].userId.S === identityId)) {
      console.log("Session expired, initiating endSession for user:", identityId);
      endSession();
    }
  }, [isAllowed, timeRemaining, identityId, users]);

  // Handle tab visibility changes
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden && credentials && identityId) {
        console.log("Tab became active, checking session for user:", identityId);
        fetchUsers();
      }
    };
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      // Update other states based on position
      const userPosition = getPositionInQueue();
      if (userPosition === 1) {
        setIsAllowed(true);
      } else {
        setIsAllowed(false);
      ...
      // Check if user was first but removed
      if (wasFirst && !sortedUsers.some((user) => user.userId.S === identityId)) {
        console.log("User was first but no longer in queue, ending session:", identityId);
        endSession();
        return;
      }
      ...
      if (sortedUsers.length === 0) {
        console.log("Queue is empty, allowing access");
        setIsAllowed(true);
        setTimeRemaining(0);
        setWasFirst(false);
        return;
      ...
      // Start session for first user if no sessionStartTime
      if (isCurrentUserFirst && sessionStartTime === 0) {
        console.log("No sessionStartTime, starting session for user:", identityId);
      ...
      if (isCurrentUserFirst) {
        ...
        setTimeRemaining(0);
        setWasFirst(false);
        console.log(`User ${identityId} is not first, position: ${getPositionInQueue()}`);
        // Remove first user if session expired
        if (elapsed >= SESSION_DURATION && sessionStartTime > 0) {
          console.log("First user session expired, removing:", firstUser.userId.S);
  ...
  const endSession = async () => {
    ...
    try {
      ...
      try {
        navigate("/InvProgresspage", { replace: true });
      } catch (navError) {
  ...
  return {
    ...
    timeRemaining,
    isLoading, // Add isLoading to return object
  };
};
