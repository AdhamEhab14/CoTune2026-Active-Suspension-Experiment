...
import "./furutaExperiment.css";
import { PubSub } from "@aws-amplify/pubsub"; // Replace with furuta?
...
function App() {
  // Change states to match the Furuta pendulum data structure
  ...
  const TABLE_NAME = "QueueTable3";
  const LQR_TABLE_NAME = "FURUTALQRparameters";

  const { isAllowed, identityId, position, timeRemaining, isLoading } =
    Queue3();
  ...
  useEffect(() => {
    ...
    if (timeRemaining < 90 && wasActive.current) {
      window.location.href = "/FurutaProgresspage";
    }
  }, [timeRemaining]);
  ...
  const A = [
    [0, 0, 1, 0],
    [0, 0, 0, 1],
    [0, 9.0048, -0.9924, -0.0343],
    [0, 128.4611, -2.055, -0.4886],
  ];
  const B = [[0], [0], [330.7834], [685.0159]];
  ...
  useEffect(() => {
    ...
    const subscription = pubsub.subscribe({ topics: "Furuta/Data" }).subscribe({
      next: (data) => {
  ...
  const handlePublish3 = async () => {
    ...
    try {
      ...
      const publishParams = {
        topic:
          "$aws/things/RaspberryPiStreamer/shadow/name/Stream2Shadow/update", // Needs to be changed to Furuta stream shadow topic
        qos: 0,
        payload: encodedPayload,
      };
      const publishCommand = new PublishCommand(publishParams);
  ...
  const handlePublish6 = async () => {
    ...
    try {
      ...
      const encodedPayload = Buffer.from(JSON.stringify(payload));
      const publishParams = {
        topic: "Esp32_RpiWakeup/sub", // Needs to be changed to Furuta wakeup topic
        qos: 0,
        payload: encodedPayload,
      };
      const publishCommand = new PublishCommand(publishParams);
  ...
  const handlePublish7 = async () => {
    ...
    try {
      ...
      const publishParams = {
        topic:
          "$aws/things/RaspberryPiStreamer/shadow/name/StreamShadow/update", // Needs to be changed to Furuta stream shadow topic
        qos: 0,
        payload: encodedPayload,
      };
      const publishCommand = new PublishCommand(publishParams);
    ...
    <div className="containerQ">
      {isLoading ? (
        ...
        <div className="containerI">
          <Helmet>
            <title>Rotary Inverted Pendulum</title>
          </Helmet>
          ...
          <div className="message-box">
            ...
            {message ? (
              <div>
                <p>Arm Angle: {message.x != null ? message.x : "N/A"}</p>
                <p>Arm Velocity: {message.xdot != null ? message.xdot : "N/A"}</p>
                <p>Pendulum Error: {message.phi != null ? message.phi : "N/A"}</p>
                <p>Pendulum Velocity: {message.phidot != null ? message.phidot : "N/A"}</p>
          ...
          <div className="chart-container">
            <div className="chart">
              <h3>Arm Angle</h3>
              <Line
                data={chartData("Arm Angle (rad)", x, "rgb(75, 192, 192)")}
                options={options}
              />
            </div>
            <div className="chart">
              <h3>Arm Velocity</h3>
              <Line
                data={chartData("Arm Velocity (rad/s)", xdot, "rgb(153, 102, 255)")}
                options={options}
              />
            </div>
          </div>
          <div className="chart-container">
            <div className="chart">
              <h3>Pendulum Error</h3>
              <Line
                data={chartData("Pendulum Error (rad)", phi, "rgb(255, 159, 64)")}
                options={options}
              />
            </div>
            <div className="chart">
              <h3>Pendulum Velocity</h3>
              <Line
                data={chartData("Pendulum Velocity (rad/s)", phidot, "rgb(255, 99, 132)")}
                options={options}
              />
            </div>
          </div>
