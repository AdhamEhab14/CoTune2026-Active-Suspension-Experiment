...
u_lb = -2500; % Input lower bounds
u_ub = 2500; % Input upper bounds

%% Road profile (bump)
V = 30 * (1000 / 3600); % Vehicle speed (m/s)
time = 0:ts:T_sim; % Road profile duration (s)
bump_L = 5; % Bump length (m)
bump_A = 0.1; % Bump height (m)
...
acc_active = y(1,1:T);  % Active suspension acceleration (using first T values of x)
