...
// --- DYNAMIC AUTO-CALIBRATION GLOBALS ---
static float g_baseline_d1_m = 0.0f;
static float g_baseline_d2_m = 0.0f;
...
static void KfTofTask(void *param) {
    ...
    for (;;) {
        ...
        if (read_ok) {
            ...
            u = g_control_force_N;
            portEXIT_CRITICAL(&g_kfMux);
            float y_tof[2] = {d1_m - g_baseline_d1_m, d2_m - g_baseline_d2_m};
...
/* %%%-SFUNWIZ_wrapper_Start_Changes_BEGIN --- EDIT HERE TO _END */
  ...
  // Allow ToF sensors a moment to start up and get valid initial readings
  delay(200);
  ...
  if (I2C_Lock(pdMS_TO_TICKS(2000))) {
      ...
      for (int i = 0; i < 50; i++) {
          if (g_lox1_ok && lox1.isRangeComplete()) {
              int32_t d = lox1.readRange();
              if (d > 0 && d < 8000) { sum_d1 += d; valid_d1++; }
          }
          if (g_lox2_ok && lox2.isRangeComplete()) {
              int32_t d = lox2.readRange();
              if (d > 0 && d < 8000) { sum_d2 += d; valid_d2++; }
          }
          delay(20); // 50 Hz ToF sampling
      }

      if (valid_d1 > 0) {
          g_baseline_d1_m = ((float)sum_d1 / valid_d1 / 1000.0f) + 0.00168f;
      } else {
          g_baseline_d1_m = 0.188f;
      }

      if (valid_d2 > 0) {
          g_baseline_d2_m = ((float)sum_d2 / valid_d2 / 1000.0f) - 0.00046f;
      } else {
          g_baseline_d2_m = 0.169f;
      }
 ...
 *
 */
extern "C" void Sensors_Outputs_wrapper(const real32_T *u_N,
			real32_T *d1_m,
			real32_T *d2_m,
			real32_T *vs_mps,
			real32_T *vus_mps);
 ...
 *
 */
extern "C" void Sensors_Terminate_wrapper(void);
